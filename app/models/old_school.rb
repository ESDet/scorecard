class OldSchool < ActiveRecord::Base
  ...
  def my_properties
    ...
    result = {
      ...
      :seal       => earlychild? ?
        OldSchool.el_image(:mini, self.esd_el_2015.andand.overall_rating) :
        OldSchool.k12_image(self.overall_grade, :small),
      :others     => self.others,
    }
    return result
  end
  ...
  def ec_specialty
    get_api_value(ecs.andand.field_ec_specialty) || earlychild.andand.gscspecialty
  end

  def ec_schedule
    get_api_value(ecs.andand.field_ec_schedule) || earlychild.andand.gscschedule
  end

  def ec_age_from
    ecs.andand.field_age_from.to_i
  end

  def ec_age_to
    ecs.andand.field_age_to.to_i
  end

  def ec_capacity
    get_api_value(ecs.andand.field_capacity) || earlychild.andand.gsccapacity
  end

  def ec_eligibility
    get_api_value(ecs.andand.field_ec_eligibility) || earlychild.andand.gsceligibility
  end

  def ec_subsidy
    get_api_value(ecs.andand.field_ec_subsidy) || earlychild.andand.gscsubsidy
  end

  def ec_special
    get_api_value(ecs.andand.field_ec_special) || earlychild.andand.gscspecial
  end

  def ec_setting
    get_api_value(ecs.andand.field_ec_setting) || earlychild.andand.gscsetting
  end

  def ec_environment
    get_api_value(ecs.andand.field_ec_environment) || earlychild.andand.environment
  end

  def ec_meals
    get_api_value(ecs.andand.field_ec_meals) || earlychild.andand.meals
  end

  def ec_pay_schedule
    get_api_value(ecs.andand.field_ec_payschedule) || earlychild.andand.gscpayschedule
  end

  def ec_fee
    get_api_value(ecs.andand.field_ec_fee).to_f || earlychild.andand.gscfee
  end
  ...
  def ec_contract
    get_api_value(ecs.andand.field_ec_contract) || earlychild.andand.gsccontract
  end

  def ec_months_of_operation
    get_api_value(ecs.andand.field_months_of_operation) || earlychild.andand.monthsofoperation
  end

  def ec_email
    get_api_value(ecs.andand.field_ec_email) || earlychild.andand.gscemail
  end

  def ec_phone
    ecs.andand.field_address.andand['phone_number'] || earlychild.andand.gscemail
  end
  ...
  def ec_message
    get_api_value(ecs.andand.field_ec_message) || earlychild.andand.gscmessage
  end
