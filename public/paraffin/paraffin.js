...
var Paraffin = {
  ...
  */
  ...
  Map: function(opts) {
    var leafOpts = $.extend({minZoom: opts.minZoom, maxZoom: opts.maxZoom, attributionControl: true }, opts.leafletOverrides || {});
    var map = new L.Map(opts.divId, leafOpts);
    this.map = map;
    this.opts = opts;
    this.spinner = null;
...
Paraffin.Map.prototype.initializeLayers = function(baseLayers, layers) {
  ...
  for(var i in baseLayers) {
    ...
    var newLayer = null;
    if(bl == 'street') {
      newLayer = baseMaps.Street = this.createLayer(Paraffin.MAPBOX);
    } else if(bl == 'satellite') {
      newLayer = baseMaps.Satellite = this.createLayer(Paraffin.MAPQUEST);
    } else if(bl == Paraffin.MIDNIGHT) { 
      that.opts.cloudmadeStyle = 999;
      newLayer = baseMaps.Street = this.createLayer(Paraffin.CLOUDMADE);
    } else {
      newLayer = baseMaps.Satellite = this.createLayer(bl);
    }
    newLayer.id = bl;
    newLayer.base = true;
  ...
  var initLayerFromBedrock = function(layer) {
    ...
    if(layer.ty == 'geojson') {
      ...
      var fullStyle = $.extend({}, that.pointDefaults, layer.style || {});

      if(layer.pointToLayer) {
        opts.pointToLayer = layer.pointToLayer;
      } else {
        opts.pointToLayer = (function(style) { return function(feature, ll) { return new L.CircleMarker(ll, style); }; })(fullStyle);
      }
      ...
      opts.onEachFeature = function (feature, featureLayer) {
        if(layer.template) {
          ...
          featureLayer.bindPopup(html, { minWidth: fullStyle.minWidth || 220 });
      ...
      overlays[layer.name].template = layer.template;
...
Paraffin.Map.prototype.createLayer = function(type, opts) {
  ...
  if(type == Paraffin.MAPBOX) {
    //var url = 'https://{s}.tiles.mapbox.com/v3/dexterslu.heod112f/{z}/{x}/{y}.png';
    //var url = 'http://a.tiles.mapbox.com/v4/anderoy.i0p181ni/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoiYW5kZXJveSIsImEiOiJESm9La25FIn0.4nD9gPa3SirZ3Odrokpqvg';
    var url = "http://a.tiles.mapbox.com/v4/esd.ExcellentSchoolsDetroit/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoiZXNkIiwiYSI6InBab1ZlUWsifQ.Gwmbd8beRpVIc2kw3xs_QA";
    var layer = new L.TileLayer(url, { maxZoom: 18 });
    return layer;
  
  } else if(type == Paraffin.CLOUDMADE) {
    ...
    if(opts.url) {
      var geojsonLayer = new L.GeoJSON(this.emptyGeoJSON, opts.options);
      $.ajax({
        async: true,
        type: 'GET',
        dataType: 'json',
      ...
      var geojsonLayer = new L.GeoJSON(opts.object, opts.options);
      if(opts.onCreate) {
        opts.onCreate(geojsonLayer);
      }
      return geojsonLayer;
...
Paraffin.Map.prototype.createTileJSON = function(opts) {
  ...
  $.ajax({ dataType: 'json', url: opts.url, data: {}, crossDomain: true, success: function(tilejson, textStatus, jqXHR) {
    ...
    if(opts.interaction && tilejson.grids) {
      ...
      var jsonp = (url.indexOf("mapbox.com") > -1);
      if(jsonp) {
       url += '?callback={cb}';
      }
      var utfGrid = new L.UtfGrid(url, { resolution: 4, useJsonP: jsonp });
      utfGrid.grid = true;
      utfGrid.imageLayer = layer; // remember what you're tied to
      
      utfGrid.on('click', function (e) {
        //click events are fired with e.data==null if an area with no hit is clicked
        if (e.data) {
          // This bit here prevents lower layers from popping up AFTER the frontmost layers
          var myZ = e.target.imageLayer.options.zIndex;
          if(e.parentEvent.firedForZ && e.parentEvent.firedForZ > myZ) {
            return;
          }
          e.parentEvent.firedForZ = myZ; // We have to store it back a level because of the way utfgrid events loop
